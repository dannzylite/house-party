...
class CurrentSong(APIView):
    def get(self, request, format=None):
        ...
        song = {
            ...
            'vote': votes,
            'vote_required':room.vote_to_skip,
            'id': song_id
        }
...
class PauseSong(APIView):
    def put(self, response, format=None):
        ...
        if self.request.session.session_key or room.guess_can_pause:
...
class SkipSong(APIView):
    def post(self, request, format=None):
        ...
        if room.host == self.request.session.session_key or len(votes) + 1 >= votes_required:
            votes.delete()
            skip_song(room.host)
        else:
